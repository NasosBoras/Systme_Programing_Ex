...
public class HostFail2 {
    ...
    private static final String CUSTOMER_SLA_CONTRACT = "SampleCustomerSLA.json";
    ...
    private HostFail2() {
        ...
        datacenter0.getHostList().forEach((h)->{
        	if(h.isFailed()) {
    ...
    private DatacenterSimple createDatacenter() {
        ...
        //Comment this if you don't want hosts failing
        hostList = failedHost(hostList);
    ...
    private Cloudlet createCloudlet(final DatacenterBroker broker, final int numberOfPes) {
        ...
        Cloudlet cloudlet = new CloudletSimple(createdCloudlets++, length, numberOfPes)
    ...
    private List<Host> failedHost(List<Host> hostList) {
    	double p = 5; //I like the number 5 :D
    	PoissonDistribution pd = new PoissonDistribution(p);
    	Iterator<Host> i = hostList.iterator();
    	int counter = 0;
    	while(i.hasNext()) {
    		Host host = i.next();
    		int sample  = pd.sample();
    		System.out.println("Poisson Distribution: "+sample);
        	if(sample<5) {
        		System.out.println("Host with ID: "+host.getId());
        		counter++;
        		host.setFailed(true);
        	}
        	if(counter == contract.getMinFaultToleranceLevel()) {
        		break;
        	}
    	}
    	return hostList;
